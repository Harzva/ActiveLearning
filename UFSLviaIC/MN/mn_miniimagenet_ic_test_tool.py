...
class MiniImageNetIC(Dataset):
    ...
    def __init__(self, data_list, transform=None,Config=''):
    ...
    def __getitem__(self, idx):
        ...
        image = Image.open(image_filename).convert(self.Config.convert)
...
class ICTestTool(object):
    ...
    def __init__(self, feature_encoder, ic_model, data_root, batch_size=64, num_workers=8, ic_out_dim=512,transform=None,Config=''):
        ...
        self.train_loader = DataLoader(MiniImageNetIC(self.data_train,Config=Config,transform=self.transform ), batch_size, False, num_workers=num_workers)
        self.val_loader = DataLoader(MiniImageNetIC(self.data_val,Config=Config,transform=self.transform ), batch_size, False, num_workers=num_workers)
        self.test_loader = DataLoader(MiniImageNetIC(self.data_test,Config=Config,transform=self.transform ), batch_size, False, num_workers=num_workers)
        pass
    ...
    def val(self, epoch, is_print=True):
        if is_print:
            self.Config.logger.info("Test {} .......".format(epoch))
            pass
        #train-KNN-top1 / total, top5 / total, acc_cluster
        ...
        if is_print:
            self.Config.logger.info("Epoch: {} KNN_train_acc {:.4f}/{:.4f} {:.4f}".format(epoch, acc_1_train , acc_2_train, acc_3_train))
            self.Config.logger.info("Epoch: {} KNN_val_acc   {:.4f}/{:.4f} {:.4f}".format(epoch, acc_1_val, acc_2_val, acc_3_val))
            self.Config.logger.info("Epoch: {} KNN_test_acc  {:.4f}/{:.4f} {:.4f}".format(epoch, acc_1_test, acc_2_test, acc_3_test))


            self.Config.writer.add_scalars('KNN_train_acc', {'top1 / total': acc_1_train,
                            'top5 / total': acc_2_train,
                            'acc_cluster':  acc_3_train}, epoch)
            self.Config.writer.add_scalars('KNN_val_acc', {'top1 / total': acc_1_val,
                            'top5 / total': acc_2_val,
                            'acc_cluster':  acc_3_val}, epoch)
            self.Config.writer.add_scalars('KNN_test_acc', {'top1 / total': acc_1_test,
                            'top5 / total': acc_2_test,
                            'acc_cluster':  acc_3_test}, epoch)
            pass
        return acc_1_val
