...
#             # pass
#             #    def __init__(self, task, transform=None, target_transform=None,split='',Config=''):
#         dataset = MiniImageNet(self.task, split=self.split, transform=self.transform,Config=self.Config)
...
class MiniImageNet(Dataset):
    ...
    def get_data_loader(task, num_per_class=1, split='train', sampler_test=False, shuffle=False, transform=None,Config=''):
        dataset = MiniImageNet(task, split=split, transform=transform,Config=Config)
        if split == 'train':
...
class TestTool(object):
    ...
    def test(self, test_avg_num, episode=0, is_print=True):
        ...
        if is_print:
            for acc in acc_list:
                self.Config.logger.info("episode={}, Test accuracy={}".format(episode, acc))
                pass
            self.Config.logger.info("episode={}, Mean Test accuracy={}".format(episode, mean_acc))
            pass
        return mean_acc
    ...
    def val(self, episode=0, is_print=True):
        ...
        if is_print:
            self.Config.logger.info("fsl_Train {} Accuracy: {}".format(episode, acc_train))
            self.Config.logger.info("fsl_Val   {} Accuracy: {}".format(episode, acc_val))
            self.Config.logger.info("fsl_Test1 {} Accuracy: {}".format(episode, acc_test1))
            self.Config.logger.info("fsl_Test2 {} Accuracy: {}".format(episode, acc_test2))
            self.Config.writer.add_scalars('fsl_acc', {'fsl_train_acc': acc_train,
                            'fsl_val_acc': acc_val,
                            'fsl_test1_acc':  acc_test1,
                            'fsl_test2_acc':  acc_test2}, episode)
            pass
        return acc_val
    ...
    def _val(self, folders, sampler_test, all_episode):
        ...
        for i in range(all_episode):
            ...
            # sample_data_loader = MiniImageNet(task,transform=self.transform,split="train",Config=self.Config).get_data_loader(
            #                                 num_per_class=1,sampler_test=sampler_test,shuffle=False)
            # batch_data_loader =MiniImageNet(task,transform=self.transform,split="val",Config=self.Config).get_data_loader(
