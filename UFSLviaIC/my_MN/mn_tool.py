...
class RunnerTool(object):
    ...
    @staticmethod
    def adjust_learning_rate3(optimizer, epoch, first_epoch, t_epoch, init_learning_rate):

        def _get_lr(_base_lr, now_epoch, _t_epoch=t_epoch, _eta_min=1e-06):
            return _eta_min + (_base_lr - _eta_min) * (1 + math.cos(math.pi * now_epoch / _t_epoch)) / 2
        ...
        if epoch < first_epoch + t_epoch * 0:  # 0-200
            ...
            learning_rate = init_learning_rate / 1000*2
        else:  # 1000-1500
            learning_rate = init_learning_rate / 10000
            pass
...
class Classifier(nn.Module):
    def __init__(self, layer_size,num_channels, nClasses = 0,  useDropout = False, image_size = 28):
...
class CNNEncoder(nn.Module):
    """docstring for ClassName   RN"""  
    def __init__(self,input_dim):
        super(CNNEncoder, self).__init__()
        self.layer1 = nn.Sequential(
                        nn.Conv2d(input_dim,64,kernel_size=3,padding=0),
...
class MatchingNet(nn.Module):
    ...
    def __init__(self, hid_dim, z_dim, input_dim,has_norm=False):
        super().__init__()
        # print('*'*60,input_dim)
        self.conv_block_1 = nn.Sequential(nn.Conv2d(input_dim, hid_dim, 3, padding=1),
                                          nn.BatchNorm2d(hid_dim), nn.ReLU(), nn.MaxPool2d(2))  # 41
        self.conv_block_2 = nn.Sequential(nn.Conv2d(hid_dim, hid_dim, 3, padding=1),
                                          nn.BatchNorm2d(hid_dim), nn.ReLU(), nn.MaxPool2d(2))  # 21
        self.conv_block_3 = nn.Sequential(nn.Conv2d(hid_dim, hid_dim, 3, padding=1),
...
class ResNet12Small(nn.Module):
    ...
    def __init__(self, block=BasicBlock, avg_pool=False, drop_rate=0.0,inplanes=3):
        super().__init__()
