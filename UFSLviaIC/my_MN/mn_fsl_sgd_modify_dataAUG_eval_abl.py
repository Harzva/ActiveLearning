...
class Runner(object):
    ...
    def load_model(self):
        ...
        print(pkl_list)
        print(os.path.dirname(Config.mn_dir))
    ...
    def matching_test(self, samples,batches,num_way_test,num_shot):
        # print('*'*60,Config.num_way_test)
        # print('*'*60,Config.num_shot)

        batch_num, _, _, _ = batches.shape
        # torch.Size([5, 512, 1, 1])
        # torch.Size([15, 512, 1, 1])
        ...
        #z_support:5*2352  batch_num 15 num_way_test 5  num_shot 1 z_dim 2352
    ...
    def eval_one(self, num_way=5, num_shot=1):
        ...
        # test_tool_fsl = FSLTestTool(self.matching_test, Config.data_root, num_way=num_way, num_shot=num_shot,
        #                         episode_size=Config.episode_size, test_episode=Config.test_episode)
        # self.test_tool = FSLTestTool(self.matching_test, data_root=Config.data_root,
        #                     num_way=Config.num_way_test,  num_shot=Config.num_shot,
        #                     episode_size=Config.episode_size, test_episode=Config.test_episode,
...
class Config(object):
    ...
    # if args.dataset not in ['omniglot', 'omniglot_single','CIFARFS', 'FC100']:
    #     print('asss')
    assert args.dataset  in ['omniglot', 'omniglot_single','CIFARFS', 'FC100'],'please right dataset'
...
################################################################################################down is same
    ...
    log_file_txt = Tools.new_dir(os.path.join("../models_abl/{}/mn/{}".format(dataset, 'result_my'),
                                                   "{}_{}.txt".format('test', 'label_conv4')))
...
if __name__ == '__main__':
    ...
    print('*'*80)
    if not args.val:
        print('you are traing')
        runner.train()
        runner.load_model()
        runner.matching_net.eval()#zhengze drop  
        runner.test_tool.val(epoch=Config.train_epoch, is_print=True)
        runner.test_tool.test(test_avg_num=5, epoch=Config.train_epoch, is_print=True)#调用 init FSLTestTool nwway正常设置即可
    else:
        print('you are eval')
        ...
        #     Tools.print("way={},shot=1,acc={},con={}".format(way, m, pm), txt_path=Config.log_file_txt)
        for index, shot in enumerate(shots):
            Tools.print("{}/{} shot={}".format(index, len(shots), shot))
            m, pm = runner.eval_one(num_way=5, num_shot=shot)
            Tools.print("way=5,shot={},acc={},con={}".format(shot, m, pm), txt_path=Config.log_file_txt)
...
def final_eval(gpu_id, name, mn_checkpoint, dataset_name, is_conv_4,
               test_episode=1000, result_dir="result", split=test:
