...
class myDataset(object):
    ...
    def __init__(self, data_list, num_way, num_shot,transform,transform_test):
        ...
        self.transform_train_ic= transform
        self.transform_train_fsl =self.transform_train_ic
        self.transform_test = transform_test


        # norm = transforms.Normalize(mean=[x / 255.0 for x in [120.39586422, 115.59361427, 104.54012653]],
        #                             std=[x / 255.0 for x in [70.68188272, 68.27635443, 72.54505529]])
        # self.transform_test_ic = transforms.Compose([transforms.CenterCrop(size=Config.image_size), transforms.ToTensor(), norm])
...
class Runner(object):
    ...
    def eval_one(self, num_way=5, num_shot=1):
        Config.logger.info("way:{} shot:{}".format(num_way, num_shot))
        Config.num_way_test=num_way
        Config.num_shot=num_shot
        self.matching_net.eval()
        self.ic_model.eval()
        test_tool_fsl = FSLTestTool(self.matching_test, data_root=Config.data_root,
                                      num_way=Config.num_way_test, num_shot=Config.num_shot,
                                      episode_size=Config.episode_size, test_episode=Config.test_episode,
                                      transform=self.task_train.transform_test,Config=Config)
        test_tool_ic = ICTestTool(feature_encoder=None, ic_model=self.ic_model,
                                       data_root=Config.data_root, batch_size=Config.batch_size,
                                       num_workers=Config.num_workers, ic_out_dim=Config.ic_out_dim,
                                       transform=self.task_train.transform_test,Config=Config)

        # test_tool_ic.val(epoch=Config.train_epoch, is_print=True)
        # test_tool_fsl.val(epoch=Config.train_epoch, is_print=True)
        test_tool_fsl.test(test_avg_num=10, epoch=Config.train_epoch, is_print=True)
    ...
    def train(self):
        ...
        for epoch in range(1, 1 + Config.train_epoch):
            ...
            Config.logger.info("Epoch:[{}] mn_lr={} ic_lr={} loss:{:.3f} fsl:{:.3f} ic:{:.3f} ok:{:.3f}({}/{})".format(epoch, mn_lr, ic_lr,all_loss / len(self.task_train_loader),
            ...
            if epoch % Config.val_freq == 0:
                ...
                val_accuracy = self.test_tool_fsl.val(epoch=epoch, is_print=True)
...
def parse_args():
    ...
    parser.add_argument('--lr',type=int, default=1,help=' lr function id')
...
class Config(object):
    ...
    if  args.fsl_backbone=='res12':
        ...
        commit=f'{num_way}w{num_shot}s_{args.fsl_backbone}_lr{args.lr}_{dataset}_{convert}'
    elif args.fsl_backbone=='res12B':
        matching_net = ResNet(avg_pool=True, drop_rate=drop_rate,inplanes=input_dim)
        commit=f'{num_way}w{num_shot}s_{args.fsl_backbone}_lr{args.lr}_{dataset}_{convert}'
...
################################################################################################down is same
    ...
    ic_dir = Tools.new_dir(f"{date_dir}/{model_name}_{head}.pkl") if not args.val else args.val
...
if __name__ == '__main__':
    ...
    if not args.val:
        ...
        runner.test_tool_fsl.val(epoch=Config.train_epoch, is_print=True)
        runner.test_tool_fsl.test(test_avg_num=5, epoch=Config.train_epoch, is_print=True)

    else:
        runner.load_model()
        runner.eval_one(num_way=20, num_shot=1)
        runner.eval_one(num_way=5, num_shot=1)
