...
parser.add_argument('--data.way','-w', type=int, default=60, metavar='WAY',help="number of classes per episode (default: 60)")
parser.add_argument('--data.shot', '-s',type=int, default=5, metavar='SHOT',help="number of support examples per class (default: 5)")
parser.add_argument('--data.query', '-q',type=int, default=5, metavar='QUERY',help="number of query examples per class (default: 5)")

parser.add_argument('--data.test_way','-tw', type=int, default=5, metavar='TESTWAY',help="number of classes per episode in test. 0 means same as data.way (default: 5)")
parser.add_argument('--data.test_shot','-ts', type=int, default=0, metavar='TESTSHOT',help="number of support examples per class in test. 0 means same as data.shot (default: 0)")
parser.add_argument('--data.test_query','-tq', type=int, default=15, metavar='TESTQUERY',help="number of query examples per class in test. 0 means same as data.query (default: 15)")
...
#python scripts/train/few_shot/run_train.py -w 5 -s 1 -q 15 -tw 5 -ts 1 -tq 15 --log.exp_dir=train-test-5w_1s_15q --data.cuda
#python scripts/train/few_shot/run_train.py --log.exp_dir=train-60w_5s_5q_test-5w_0s_5q --data.cuda
# model args
